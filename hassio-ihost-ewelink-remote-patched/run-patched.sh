...
log_info "Patched eWeLink-Remote entrypoint: skipping core_mosquitto service dependency"
...
log_info "Waiting for MQTT broker authorization before starting eWeLink-Remote app..."
...
if [ -f /workspace/dist/app.js ]; then
    log_info "Starting eWeLink-Remote application with fastify dist/app.js"
    exec fastify start -l info -p 3031 -a 0.0.0.0 /workspace/dist/app.js
fi

if [ -f /workspace/src/server.js ]; then
    log_info "Starting eWeLink-Remote application with node /workspace/src/server.js"
    exec node /workspace/src/server.js
fi

if [ -f /workspace/src/index.js ]; then
    log_info "Starting eWeLink-Remote application with node /workspace/src/index.js"
    exec node /workspace/src/index.js
fi

if [ -f /workspace/server.js ]; then
    log_info "Starting eWeLink-Remote application with node /workspace/server.js"
    exec node /workspace/server.js
fi

if [ -f /workspace/index.js ]; then
    log_info "Starting eWeLink-Remote application with node /workspace/index.js"
    exec node /workspace/index.js
fi

if [ -f /app/dist/app.js ]; then
    log_info "Starting eWeLink-Remote application with fastify /app/dist/app.js"
    exec fastify start -l info -p 3031 -a 0.0.0.0 /app/dist/app.js
fi

if [ -f /app/src/server.js ]; then
    log_info "Starting eWeLink-Remote application with node /app/src/server.js"
    exec node /app/src/server.js
fi

if [ -f /app/src/index.js ]; then
    log_info "Starting eWeLink-Remote application with node /app/src/index.js"
    exec node /app/src/index.js
fi

if [ "$#" -gt 0 ]; then
    log_info "Starting eWeLink-Remote application with original CMD"
    exec "$@"
fi

log_error "No known eWeLink-Remote start command found"
log_info "Directory listing for diagnostics:"
find /workspace /app -maxdepth 3 -type f 2>/dev/null | sed 's#^#  #' || true
exit 1
